...
var extractText = function( filePath, cb ) {
  ...
  catdoc.on( 'close', function( code ){
    if ( error ) {
      error = new Error( "dwgdxftotext read of .dxf file named [[ " + filePath + " ]] failed: " + error.message );
      cb( error, null );
      return;
    }
    cb( null, result.trim() );
  ...
  exec('catdoc ' + __filename,
    function (error, stdout, stderr) {
      if (error !== null) {
        ...
        console.error( "textract: 'dwgdxftotext' does not appear to be installed, so it will be unable to extract DXFs." );
      }
  });
}
