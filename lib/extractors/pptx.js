...
var extractText = function( filePath, options, cb ) {
  ...
  var execOptions = util.createExecOptions("pptx", options);

  exec( "unzip -c " + filePath + " ppt/slides/*.xml",
    execOptions,
    function ( error, stdout, stderr ) {
      if ( error !== null ) {
        error = new Error( "extract powerpoint, pptx, exec error " + path.basename( filePath ) + ": " + error.message );
        cb( error, null );
        return;
      }
